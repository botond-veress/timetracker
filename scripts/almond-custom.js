...
(function () {
    (function (p) {
        var y = this || (0, eval)("this"), w = y.document, M = y.navigator, u = y.jQuery, E = y.JSON; (function (p) { "function" === typeof define && define.amd ? define('knockout',["exports", "require"], p) : "function" === typeof require && "object" === typeof exports && "object" === typeof module ? p(module.exports || exports) : p(y.ko = {}) })(function (N, O) {
...
define('helpers/bindings',[],
    function () {

        ko.bindingHandlers.disableScroll = {
            update: function (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
                var $element = $('body');
                var value = ko.unwrap(valueAccessor());
                var name = 'modal-open';
                if (value && !$element.hasClass(name)) {
                    $element.addClass(name);
                } else if (!value && $element.hasClass(name)) {
        ...
        ko.bindingHandlers.collapse = {
            init: function (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
                var $element = $(element);
                var $item = $element.children('.title');
                var $content = $element.children('.content');
                var name = 'expanded';

                $item.bind('click', toggle);
                if (!$content.hasClass(name)) {
                    $content.slideUp(0);
                }

                ko.utils.domNodeDisposal.addDisposeCallback(element, function () {
                    $item.unbind('click', toggle);
                });
        ...
        ko.bindingHandlers.material = {
            init: function (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
                $(document).on('click', '[data-material]', function (event) {
                    var $trigger = $(this);
                    var $ripple = $trigger.hasClass('material')
                        ? $trigger
                        : $trigger.parents('.material').length
                            ? $trigger.parents('.material')
                            : $trigger.children('.material');
                    if ($ripple.length) {
                        ripple($ripple, $trigger, event);
                    }
                });

                function ripple($element, $trigger, event) {
                    var $ink = $element.children('.ink');
                    if (!$ink.length) {
                        $ink = $('<div>').addClass('ink');
                        var additional = $trigger.data('material-class');
                        if (additional) {
                            $ink.addClass(additional);
                        }
                        $element.prepend($ink);
                    }
                    ...
                    if (!$ink.width() || !$ink.height()) {
                        var max = Math.max($element.outerWidth(), $element.outerHeight());
                        $ink.css({ height: max, width: max });
                    }

                    var x = event.pageX - $element.offset().left - $ink.width() / 2;
                    var y = event.pageY - $element.offset().top - $ink.height() / 2;

                    $ink.css({ top: y, left: x }).addClass('animate');
        ...
        function scroll($target, cancelable) {
            var duration = 1000;
            var distance = 2000;
            var $document = $(window);
            ...
            $('body, html').stop().animate({
                scrollTop: top + 1
            }, getDuration(top, duration, distance), function () {
                if (cancelable && !canceled) {
                    $document.unbind('scroll DOMMouseScroll mousewheel keyup', stop);
                }
            });

            if (cancelable) {
                $document.one('scroll DOMMouseScroll mousewheel keyup', stop);
            }
            ...
            function getDuration(top, duration, distance) {
                return Math.max(Math.abs(top - $document.scrollTop()) * duration / distance, 250);
            }
        }

        function getOffsetTop($target) {
            return $target.offset().top - $('#header').outerHeight();
        }

        ko.bindingHandlers.scrollCallback = {
            init: function (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
                var $document = $(window);
                ...
                function check() {
                    ...
                    if (typeof value.callback == 'function') {
                        var percent = ko.utils.unwrapObservable(value.percent) || null;
                        var offset = !percent ? ko.utils.unwrapObservable(value.offset) || null : null;
                        var current = $document.scrollTop();
                        var passed = percent
                            ? (current * 100 / ($page[0].scrollHeight - $document.outerHeight())) > percent
                            : current > offset;
                        value.callback(passed);
                    }
                }

                $(document).scroll(check);
                $(document).resize(check);
            }
        };

        ko.bindingHandlers.scroll = {
            update: function (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
                var section = ko.utils.unwrapObservable(valueAccessor());
                if (section) {
                    var $target = $('#' + section);
                    if ($target.length) {
                        scroll($target, true);
                    }
                    viewModel.navigate(section, !($target && $target.length) || ($('[href="#page/' + section + '"]').length == 0));
                }
            }
        };
            ...
            init: function (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
                ...
                if (ko.isObservable(value) && ko.isWriteableObservable(value)) {
                    ...
                    $element.click(function (event) {
                        ...
                        var old = ko.utils.unwrapObservable(value);
                        if (old == section) {
                            value(null);
                        }
                        value(section);
                    ...
                    $(document).scroll(activate);
                    $(document).resize(activate);
                }

                function refreshTarget() {
                    hash = $element.attr('href');
                    section = getSectionFromHash(hash);
                    $target = section
                        ? $('#' + section)
                        : null;
                }
                ...
                function isInsideOfViewport() {
                    var last = $element.is(':last-child');
                    if ($target && $target.length) {
                        var half = Math.max($document.height() / 2, 300);
                        var top = getOffsetTop($target) - $document.scrollTop();
                        var height = $target.outerHeight();
                        return half > top && (half < top + height || last);
                    }
                    return false;
                }
            }
...
define('main',['durandal/system', 'durandal/app', 'durandal/viewLocator', 'plugins/router', 'jquery', 'knockout', 'helpers/bindings'],
